#!/usr/bin/env python3
"""
noise.py — Test model robustness against 10% noise variants.

Noise variants (generated by add_noise.py):
    saltpepper  data_10          (halves of 10% pixels → 1.0 / 0.0)
    gaussian    data_10_gaussian (additive gaussian, σ = 0.2 × band std)
    stripe      data_10_stripe   (vertical stripes, per-column offsets)
    band        data_10_band     (10% of bands replaced by noise)

Compares performance on clean data vs each noise variant for all
architectures. Loads clean results from existing benchmark output, then
runs each architecture with each noise variant and compares.

Usage:
    python noise.py
    python noise.py --food Almond
    python noise.py --only gthad otad
    python noise.py --type gaussian stripe
...
from add_noise import out_stem
...
def noise_suffix(noise_type):
    """Weight suffix: keep '_10' for saltpepper (reuses old weights), '_<type>' otherwise."""
    if noise_type == "saltpepper":
        return "_10"
    return f"_{noise_type}"


def generate_noise_data(food_types, noise_types=None):
    """Generate data_10{_type}.hdr + *.noise_mask.npy for each noise type if missing."""
    from add_noise import generate
    if noise_types is None:
        noise_types = NOISE_TYPES
    generate(food_types=food_types, noise_types=noise_types, seed=NOISE_SEED)


@contextlib.contextmanager
def swap_to_noisy(food_types, noise_type):
    """Swap data.hdr ↔ data_10{_type}.hdr using .bak files in the same directory."""
    ...
    try:
        for ft in food_types:
            for split in ["Train", "Test"]:
                ...
                noisy_hdr = os.path.join(base, f"{stem}.hdr")
                noisy_raw = os.path.join(base, stem)
                ...
                print(f"  📦 {ft}/{split}: data.hdr → .clean_bak, {stem}.hdr → data.hdr")
...
def print_comparison_summary(all_results):
    """Print comparison table: clean vs noisy metrics per food type for all noise types."""
    ...
    noise_types = [c for c in NOISE_TYPES if f"noisy_{c}" in conditions]
    ...
    for ft in food_types:
        ...
        for arch in sorted(archs):
            ...
            for noise_type in noise_types:
                noisy_key = f"{arch}|noisy_{noise_type}|{ft}"
...
def plot_noise_1x3(arch, food_type, noise_type, output_dir, anomaly_dir):
    ...
    mask_path = os.path.join(base_path, f'{stem}.noise_mask.npy')
    ...
    for name, arr, cmap in [('gt', gt_binary, 'hot'), ('noise_mask', noise_mask, 'gray'), ('predicted', predicted, 'hot')]:
        ...
        ax.axis('off')
        save_path = os.path.join(out_dir, f'{name}_{arch}_{food_type}_{noise_type}.png')
        plt.tight_layout()
    ...
    axes[1].set_title(f'Noise Mask ({noise_type}, {NOISE_PCT}%)')
    axes[1].axis('off')
    ...
    save_path = os.path.join(out_dir, f'noise_1x3_{arch}_{food_type}_{noise_type}.png')
    plt.tight_layout()
    plt.savefig(save_path, dpi=150, bbox_inches='tight')
    plt.close()
...
def main():
    parser = argparse.ArgumentParser(
        description=f"Compare model performance: clean vs {NOISE_PCT}% noise variants "
                    f"({', '.join(NOISE_TYPES)}).",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python noise.py                          # Run all architectures with all noise types
  python noise.py --food Almond            # Test on Almond only
  python noise.py --only gthad otad        # Run only GT-HAD and OTAD
  python noise.py --type gaussian stripe   # Run only gaussian and stripe noise
  python noise.py --dry-run                # Validate pipeline (1 epoch/iter per arch)
    ...
    parser.add_argument(
        ...
        default=None,
        metavar="TYPE",
        help=f"Noise types to run: {NOISE_TYPES} (default: all)",
    )
    parser.add_argument(
        "--dry-run",
        "-dr",
    ...
    if args.type:
        for nt in args.type:
            if nt not in NOISE_TYPES:
                print(f"❌ Unknown noise type: {nt}. Available: {NOISE_TYPES}")
                sys.exit(1)
        noise_types = [nt for nt in NOISE_TYPES if nt in args.type]
    else:
    ...
    print(f"\n{'=' * 100}")
    print(f"  HSI Food Anomaly - Noise Robustness Test ({NOISE_PCT}% noise variants){dry_run_str}")
    ...
    # Generate all noise types if missing
    print(f"\n{'─' * 60}")
    ...
    # Run noise tests for each noise type
    for noise_type in noise_types:
        type_dir = os.path.join(args.output_dir, noise_type)
        os.makedirs(type_dir, exist_ok=True)

        anomaly_dir = os.path.join(type_dir, 'anomaly_maps')
        os.makedirs(anomaly_dir, exist_ok=True)
        extra_kwargs["common"]["anomaly_dir"] = anomaly_dir

        print(f"\n{'#' * 100}")
        print(f"# NOISE TYPE: {noise_type} ({NOISE_PCT}%){' ' * 50}")
        print(f"{'#' * 100}")
        ...
        for arch in archs:
            ...
            if args.retrain == "yes":
                extra_kwargs[arch] = {"noise_suffix": noise_suffix(noise_type)}

            print(f"\n  🔄 [{arch}] swapping to {out_stem(noise_type)}.hdr ({noise_type})...")
            with swap_to_noisy(food_types, noise_type):
            ...
            for ft in food_types:
                plot_noise_1x3(arch, ft, noise_type, args.output_dir, anomaly_dir)

            for ft, result in noisy_results.items():
                all_results[f"{arch}|noisy_{noise_type}|{ft}"] = result

            # Save per-architecture noise results
            arch_path = os.path.join(type_dir, f"{arch}.json")
            with open(arch_path, "w") as f:
                json.dump(noisy_results, f, indent=2, default=convert_numpy)
            print(f"  💾 {arch} results → {arch_path}")

            elapsed = time.time() - arch_start
            print(f"\n⏱  [{arch}] {noise_type} noise wall time: {elapsed:.1f}s")
        ...
        # Save combined results for this noise type (clean + noisy_{type})
        combined = {}
        for k, v in all_results.items():
            parts = k.split("|", 2)
            if len(parts) == 3:
                cond = parts[1]
                if cond == "clean" or cond == f"noisy_{noise_type}":
                    combined[k] = v
        combined_path = os.path.join(type_dir, "noise_all.json")
    ...
    for candidate_arch in MAIN_ARCHS:
        ...
        if os.path.isfile(all_path):
            ...
            for noise_type in noise_types:
                for ft in food_types:
                    key = f"{candidate_arch}|noisy_{noise_type}|{ft}"
