"""
PA2E (Partial-window Autoencoder with DSVDD) Benchmark
...
Inference: DSVDD center distance anomaly scoring

Usage:
    from scripts.pa2e.__main__ import benchmark_food_type
...
def train_phase2(model, train_loader, val_loader, num_epochs=500, lr=1e-3, alpha=0.5, food_type=None):
    """
    Phase 2: DSVDD-style manifold refinement + CosineAnnealingLR + Early Stopping.
    
    Note: num_epochs is a hard upper limit, but early stopping will typically terminate earlier.
    Saves model at epoch 20 if training reaches it, then saves final model.
    """
...
def inference_dsvdd(model, test_loader, device, H, W):
    """Run DSVDD center distance inference on test set. Returns (scores, elapsed_time)."""
    print("\n--- DSVDD Inference (Center Distance) ---")
    start = datetime.now()
    ...
    with torch.no_grad():
        for batch in tqdm(test_loader, desc="Computing DSVDD distances"):
            # Unpack batch (test_loader yields (features, labels))
            x = batch[0] if isinstance(batch, (list, tuple)) else batch
            x = x.to(device)
            distances = model.compute_anomaly_score(x)
...
def benchmark_food_type(food_type: str,
    ...
    Returns:
        dict with results including original_model and calibration_model metrics
    """
    ...
    # Flatten labels (convert to binary: 0=anomaly, 1=normal)
    test_labels_flat = test_labels.reshape(-1)
    binary_labels = (test_labels_flat != 2).astype(int)  # Assuming label 2 is normal
    ...
    print(f"\n{'='*80}")
    print(f"Final Model Results: ROC-AUC={roc_auc:.4f}, PR-AUC={pr_auc:.4f}")
