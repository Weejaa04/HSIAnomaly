...
from scripts.otad.model import OTADNet, OTADDataset, BlockRestore
from scripts_agri.otad.utils import get_food_data, SEED_DICT, UniversalEarlyStopping
...
def benchmark_food_type(
    ...
    **kwargs,
    ...
    img_train, img_tensor, gt, H, W, B, train_mask, val_mask = get_food_data(food_type, base_dir, device=device, split_method=split_method)
    ...
    data_set_train = OTADDataset(img_train, block_size=block_size, stride=stride, spatial_mask=train_mask)
    data_set_val = OTADDataset(img_train, block_size=block_size, stride=stride, spatial_mask=val_mask)
